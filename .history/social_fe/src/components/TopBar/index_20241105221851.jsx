  ...
  return (
    <div className="flex items-center justify-between w-full px-4 py-3 topbar md:py-6 bg-primary">
      ...
      >
        <TextInput
          placeholder="Search..."
          styles="w-[18rem] lg:w-[38rem] mx-auto rounded-full py-3"
          register={register("search")}
        />
      </form>

      {/* icons */}
      <div className="flex items-center gap-4 px-10 text-base text-ascent-1 md:text-xl">
          ...
          <PopoverPanel className="absolute right-0 z-10 p-3 transition-opacity duration-200 ease-in-out bg-white rounded-lg shadow-lg w-[200px] h-[110px] ">
            <div className="flex flex-col items-start p-1 space-y-2 text-sm cursor-pointe ">
              <div className="flex items-center gap-2">
                <FaRegUserCircle />
                <Link to='/profile' className="w-full font-semibold text-left text-gray-500 rounded-lg hover:text-blue-400 ">Trang cá nhân</Link>
              </div>
              <div className="flex items-center gap-2" >
                <IoSettingsOutline />
                <Link to='/setting' className="w-full font-semibold text-left text-gray-500 hover:text-blue-400 ">Settings</Link>
              </div>
              <div className="flex items-center gap-2">
                <MdOutlineLogout />
                <Link
                  onClick={() => dispatch(Logout())}
                  className="font-semibold hover:text-red-400">Đăng Xuất</Link>
              </div>
